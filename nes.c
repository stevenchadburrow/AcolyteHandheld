...
volatile unsigned char __attribute__((coherent, aligned(8))) cart_header[16]; // copy of ROM header for quicker access
...
volatile unsigned char nes_palette_single[64] = {
...
volatile unsigned short nes_palette_double[64] = {
...
// change for platform
void nes_pixel_vga_pal(unsigned short pos_x, unsigned short pos_y, unsigned char color)
{
	nes_pixel_loc = ((pos_y))*SCREEN_X+((pos_x*3))+(1-screen_frame)*SCREEN_XY;
	nes_pixel_val = nes_palette_single[color];
	screen_buffer[(nes_pixel_loc)] = (unsigned char)nes_pixel_val;
	screen_buffer[(nes_pixel_loc+1)] = (unsigned char)nes_pixel_val;
	screen_buffer[(nes_pixel_loc+2)] = (unsigned char)nes_pixel_val;
}

// change for platform
void nes_pixel_vga_raw(unsigned short pos_x, unsigned short pos_y, unsigned char color)
{
	nes_pixel_loc = ((pos_y))*SCREEN_X+((pos_x*3))+(1-screen_frame)*SCREEN_XY;
	screen_buffer[(nes_pixel_loc)] = (unsigned char)color;
	screen_buffer[(nes_pixel_loc+1)] = (unsigned char)color;
	screen_buffer[(nes_pixel_loc+2)] = (unsigned char)color;
}

// change for platform
void nes_pixel_lcd_pal(unsigned short pos_x, unsigned short pos_y, unsigned char color)
{
	nes_pixel_loc = (((SCREEN_Y-pos_y-1)*2))+((pos_x))*SCREEN_Y2+(1-screen_frame)*SCREEN_XY;
	nes_pixel_val = nes_palette_double[color];
	screen_buffer[(nes_pixel_loc)] = (unsigned char)((nes_pixel_val&0xFF00)>>8);
	screen_buffer[(nes_pixel_loc+1)] = (unsigned char)(nes_pixel_val&0x00FF);
}
...
{
	nes_pixel_loc = (((SCREEN_Y-pos_y-1)*2))+((pos_x))*SCREEN_Y2+(1-screen_frame)*SCREEN_XY;
	screen_buffer[(nes_pixel_loc)] = (unsigned char)((color&0xFF00)>>8);
	screen_buffer[(nes_pixel_loc+1)] = (unsigned char)(color&0x00FF);
}
					...
					if (pixel_color != 0x00)
					{
						...
						if (pixel_x >= 0 && pixel_x < 256)
						{
							if (ppu_flag_lb > 0 || tile*8+i >= 8) 
							{
								if (ppu_flag_g > 0)
								{
									nes_pixel_vga_pal(pixel_x, pixel_y, (pal_ram[(((pixel_table&0x03)<<2)+pixel_color)]&0x30));
								}
								else
								{
									nes_pixel_vga_pal(pixel_x, pixel_y, pal_ram[(((pixel_table&0x03)<<2)+pixel_color)]);
								}
							}
						}
					}
